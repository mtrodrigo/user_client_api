...
export default class UserController {
  ...
  static async updateUser(req, res) {
    ...
    if (emailExists) {
      return res.status(422).json({ message: "Email already in use by another user" });
    }
    ...
    if (cpfCnpjExists) {
      return res.status(422).json({ message: "CPF/CNPJ already registered for another user" });
    }
    ...
    if (password) {
      if (password !== confirmpassword) {
        return res.status(422).json({ message: "Password and confirmation are different" });
      }
